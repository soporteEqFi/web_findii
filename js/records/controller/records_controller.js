...
const Controller = {
    ...
    async mostrarDatos(){
        ...
        // Combinar datos por cada solicitante
            ...
            const agente = agents.find(a => a.asesor_id === solicitante.id) || {};
            const actividad = activity.find(a => a.solicitante_id === solicitante.id) || {};
            const finanzas = financial.find(f => f.solicitante_id === solicitante.id) || {};
            const ubicacion = location.find(l => l.solicitante_id === solicitante.id) || {};
            const producto = product.find(p => p.solicitante_id === solicitante.id) || {};
            const solicitudInfo = solicitud.find(s => s.solicitante_id === solicitante.id) || {};

                return {
                    //info solicitante
                    id_solicitante: solicitante.id || "N/A",
                    ...
                    //location
                    ciudad: ubicacion.ciudad_gestion || "N/A",
